...
export function Historico() {
  ...
  function toggleDropdown(id: any, solId: any, newStatus: any) {
    ...
    if (!mostrarDropdown) {
      HistoryService.historyByIdAndNotStatus(solId, newStatus).then((res) => {
  ...
  const loadData = (page: number) => {
    HistoryService.page(page, rowsPerPage, orderBy, asc).then((response) => {
                ...
                <>
                  <TableRow key={h.id}>
                    <TableCell scope="row">
                      <CustomStatus
                        text={
                          h.newStatus === 'PENDING_ADVISOR'
                            ? 'Aguardando Confirmação'
                            : h.newStatus === 'PENDING_LAB'
                  ...
                  <TableRow style={{ paddingBottom: 0, paddingTop: 0 }}>
                    ...
                    >
                      <Collapse
                        in={mostrarDropdown && selectedSolicitation === h.id}
                        timeout="auto"
                        unmountOnExit
                      >
                        <Table>
                            ...
                            {historyItens?.map((i: SolicitationAudit) => (
                              <TableRow key={i.id}>
                                <TableCell scope="row">
                                  <CustomStatus
                                    text={
                                      i.newStatus === 'PENDING_ADVISOR'
                                        ? 'Aguardando Confirmação'
                                        : i.newStatus === 'PENDING_LAB'
                                        ? 'Aguardando Laboratório'
                                        : i.newStatus === 'PENDING_SAMPLE'
                                        ? 'Aguardando Amostra'
                                        : i.newStatus === 'APPROVED'
                                        ? 'Aguardando Análise'
                                        : i.newStatus === 'PENDING_PAYMENT'
                                        ? 'Aguardando Pagamento'
                                        : i.newStatus === 'REFUSED'
                                        ? 'Recusado'
                                        : i.newStatus === 'FINISHED'
                                        ? 'Concluído'
                                        : '#000000'
                                    }
                                    padding="0.5rem"
                                ...
                                <TableCell scope="row">
                                  {new Date(i.changeDate).toLocaleString(
                                    'en-GB',
                                    { timeZone: 'UTC' }
                                  )}
                                </TableCell>
