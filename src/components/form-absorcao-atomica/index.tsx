...
export const FormAbsorcaoAtomica: React.FC<FormProps> = (props: Readonly<FormProps>) => {
  ...
  const [absorcaoAtomica, setAbsorcaoAtomica] = useState<any>({
    nomeAluno: "",
    nomeOrientador: "",
    projeto: 1,
    descricao: "",
  ...
  useEffect(() => {
    console.log('props.solicitation', props.solicitation)
    if (props.solicitation) {
      const solicitation = props.solicitation;
      let fields;
      try {
        if (solicitation.fields) {
          fields = JSON.parse(solicitation.fields);
        }
      } catch (error) {
        console.log('Erro ao converter JSON do campo fields da solicitação.', error);
      }
      if (solicitation && fields) {
        setAbsorcaoAtomica(
          {
            ...
            limites: fields.limites,
            condicoes: fields.condicoes,
            elementos: fields.elementos,
            concentracao: fields.concentracao,
            observacoes: fields.observacoes,
          }
        );
      } else {
        setAbsorcaoAtomica(
          { // TODO: refatorar pois na var absorcaoAtomica usa o mesmo objeto
            nomeAluno: "",
            nomeOrientador: "",
            projeto: 1,
            descricao: "",
      ...
      console.log('setAbsorcaoAtomica', absorcaoAtomica)
    }
  }, []);
